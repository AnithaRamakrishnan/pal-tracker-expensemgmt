...
namespace IntegrationTest
{
    public class FlowTest : IDisposable
    {
        ...
        private readonly AppServerBuilder.AppServer _expensemanagement;
        ...
        public FlowTest()
        {
            ...
            _expensemanagement = TestAppServerBuilder()
               .AppName("ExpenseManagement")
               .Port(8882)
               .Database("tracker_expensemanagement_dotnet_test")
               .SetEnvironmentVariable("REGISTRATION_SERVER_ENDPOINT", _registrationServer.Url())
               .SetEnvironmentVariable("EUREKA__CLIENT__SHOULDFETCHREGISTRY", "false")
               .SetEnvironmentVariable("DISABLE_AUTH", "true")
               .SetEnvironmentVariable("SPRING__CLOUD__CONFIG__ENABLED", "false")
        ...
        public void TestBasicFlow()
        {
            ...
            response = _httpClient.Get(_expensemanagement.Url());
            Assert.Equal("Noop!", response.Content.ReadAsStringAsync().Result);

            var createdExpenseId = _httpClient.Post(_expensemanagement.Url("/expense"), new Dictionary<string, object>
            {
                {"projectId", createdProjectId},
                {"userId", createdUserId},
                {"dates", "2015-12-17"},
            ...
            AssertGreaterThan(createdExpenseId, 0);

            response = _httpClient.Get(_expensemanagement.Url($"/expense?projectId={createdProjectId}"));
            AssertNotNullOrEmpty(response.Content.ReadAsStringAsync().Result);
            Assert.True(response.IsSuccessStatusCode);
        }
